...
class HssdRetrievalApp:
    """ASGI app for HSSD retrieval with round-robin scheduling."""
    ...
    def _get_retriever(self) -> HssdRetriever:
        if self._retriever is None:
            ...
            data_path = Path(
                self._hssd_data_path
                or os.environ.get("HSSD_DATA_PATH", "data/hssd-models")
            )
            preprocessed_path = Path(
                self._hssd_preprocessed_path
                or os.environ.get("HSSD_PREPROCESSED_PATH", "data/preprocessed")
            )
    ...
    def _retrieve_objects_endpoint(self, data: list[dict[str, Any]]):
        ...
        batch_requests = [HssdRetrievalServerRequest(**req) for req in data]
