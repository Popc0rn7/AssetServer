...
from assetserver.articulated_retrieval.config import ArticulatedConfig
...
class ArticulatedRetrievalServer:
    """
    Manages the lifecycle of a Articulated retrieval server with proper resource
    management and clean shutdown capabilities.

    Unlike the BlenderServer, this doesn't need subprocess isolation since
    Articulated retrieval with CLIP doesn't have the same thread-safety requirements as bpy.
    ...
    Example:
        >>> server = ArticulatedRetrievalServer(host="127.0.0.1", port=7001)
        >>> server.start()
    ...
    def __init__(
        ...
        Args:
            ...
            preload_retriever: Whether to preload the Articulated retriever (includes CLIP
            ...
            articulated_data_path: Path to Articulated models directory. If None, uses environment
                variable Articulated_DATA_PATH or default "data/articulated-models".
            articulated_preprocessed_path: Path to preprocessed data directory. If None,
                uses environment variable Articulated_PREPROCESSED_PATH or default
                "data/preprocessed".
            articulated_top_k: Number of top CLIP candidates before size ranking (default: 5).
