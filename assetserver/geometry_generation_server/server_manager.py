...
class GeometryGenerationServer:
    ...
    def __init__(
        ...
        Args:
            ...
            backend: Which 3D generation backend to use ("sam3d" or "hunyuan3d").
                Default: "sam3d".
            ...
            startup_timeout_s: Maximum time to wait for the HTTP server to become
                ready. SAM3D preloading can take several minutes on multi-GPU hosts
                because workers serialize model initialization.
    ...
    def start(self) -> None:
        ...
        the FastAPI HTTP server. Worker processes preload the pipeline if
        ...
        try:
            # Create the FastAPI application with all parameters.
            ...
            self._wait_until_ready(timeout=self._startup_timeout_s)
            self._running = True
