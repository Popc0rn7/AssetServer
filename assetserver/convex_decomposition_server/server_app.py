"""FastAPI application for mandatory convex decomposition."""
...
class ConvexDecompositionServerApp:
    """ASGI server for convex decomposition (CoACD and V-HACD).
    ...
    def __init__(self) -> None:
        self.app = FastAPI(title="AssetServer Convex Decomposition")
        self.app.add_api_route(
            "/", self._root_endpoint, methods=["GET"], response_class=HTMLResponse
        )
        self.app.add_api_route("/health", self._health_endpoint, methods=["GET"])
        self.app.add_api_route(
            "/generate_collision", self._generate_collision_endpoint, methods=["POST"]
        )
    ...
    def _health_endpoint(self) -> dict[str, str]:
        """Health check endpoint."""
        return {"status": "healthy"}

    def _generate_collision_endpoint(self, data: dict) -> dict:
        ...
        try:
            ...
            if not mesh_path_str:
                raise HTTPException(
                    status_code=400, detail="Missing mesh_path parameter"
                )

            mesh_path = Path(mesh_path_str)
            if not mesh_path.exists():
                raise HTTPException(
                    status_code=400, detail=f"Mesh file not found: {mesh_path}"
                )
